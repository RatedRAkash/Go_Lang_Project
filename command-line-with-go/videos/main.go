package main

import (
	"fmt"
	"flag"
	"os"
)

func main(){
	// in "Go" if any Variable which is Outside the package & contains "Capital Letter" in it's Name then that Variable has become Global

	// videosList := getVideos()
	
	// saveVideos(videosList)
	// fmt.Println("Hola world")

	// fmt.Println(videosList)

	getCmd := flag.NewFlagSet("get", flag.ExitOnError)

	getAll := getCmd.Bool("all", false, "Get all videos")
	getID := getCmd.String("id", "", "Youtube video ID")

	addCmd := flag.NewFlagSet("add", flag.ExitOnError)

	addID := addCmd.String("id", "", "Video ID")
	addTitle := addCmd.String("title", "", "Youtube video Title")
	addUrl := addCmd.String("url", "", "Youtube video URL")
	addDesc := getCmd.String("desc", "", "Youtube video Description")

	if len(os.Args) < 2 {
		fmt.Println("expected 'get' or 'add' subcommands")
		os.Exit(1)
	}

	//look at the 2nd argument's value
	switch os.Args[1] {
		case "get": // if its the 'get' command
			HandleGet(getCmd, getAll, getID)
		case "add": // if its the 'add' command
			HandleAdd(addCmd, addID, addTitle, addUrl, addDesc)
		default: // if we don't understand the input
	}
}

func HandleGet(getCmd *flag.FlagSet, all *bool, id *string){
	getCmd.Parse(os.Args[2:])

	if *all == false && *id == "" {
    fmt.Print("id is required or specify --all for all videos")
    getCmd.PrintDefaults()
    os.Exit(1)
  }

	if *all {
		//return all videos
		videos := getVideos()
		
		fmt.Printf("ID \t Title \t URL \t Description \n")
		for _, video := range videos {
			fmt.Printf("%v \t %v \t %v \t %v \n",video.Id, video.Title, video.Url, video.Description)
		}

		return
	}

	if *id != "" {
		videos := getVideos()
		id := *id
		for _, video := range videos {
			if id == video.Id {
				fmt.Printf("ID \t Title \t URL \t Description \n")
				fmt.Printf("%v \t %v \t %v \t %v \n",video.Id, video.Title, video.Url, video.Description)
			}
		}
	}
}

func ValidateVideo(addCmd *flag.FlagSet,id *string, title *string, url *string, description *string ){

	addCmd.Parse(os.Args[2:])

	if *id == "" || *title == "" || *url == "" || *description == "" {
		fmt.Print("all fields are required for adding a video")
		addCmd.PrintDefaults()
		os.Exit(1)
	}

}

func HandleAdd(addCmd *flag.FlagSet, id *string, title *string, url *string, desc *string){
	ValidateVideo(addCmd, id, title, url, desc)

	video := video{
		Id: *id,
		Title: *title,
		Description: *desc,
		Url: *url, 
	}

	videos := getVideos()
	videos = append(videos, video)

	saveVideos(videos)
}